...
class NaiveBayes:
	...
	def test_looper(self,testing_data):
		...
		for instance in testing_data:
			...
			print predicted_class+" "+actual_class+" "+format_number("{0:.12f}".format(posterior_probability))
...
#	testing_data = read_file(testing_filename,False)
        #print 'OREDERED FEATURES'
        #print features_values
        #first parameter :: train set
        training_filename = sys.argv[1]
        fpr = open(training_filename,"r")
        #second parameter :: test set
        testing_filename = sys.argv[2]
        fpt = open(testing_filename,"r")

        train_data, train_meta = loadarff(fpr)
        ...
        test_data, test_meta = loadarff(fpt)
        test_instances = [list(i) for i in test_data]
